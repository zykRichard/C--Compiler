...
void before_func(int ifmain){
    ...
    fprintf(fp, "addi $sp, $sp, -%d\n", ifmain ? FunStackSZ[0] : FunStackSZ[CurFunNum]);
...
void TransFunctions(){
    ...
    while(cur != NULL){
        ...
        case FUNCTION_IR: {
            ...
            memset(vis, 0, InterCodes -> TempVarNum_t * sizeof(int));
        }
            break;
...
void MIPSGeneratorInit(){
    ...
    TransFunctions();
    memset(VarInfoRec, 0, InterCodes -> TempVarNum_t * sizeof(VarInfo));
...
void MIPSGenerator(){
    ...
    while(curic != NULL){
        ...
        {
        case LABEL_IR:
            fprintf(fp, "%s:", IRcode -> IR_Type.Single.op -> id.name);
            break;
